...
$configFile = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'OAuth 2.0  ID.txt';
$configText = is_readable($configFile) ? file_get_contents($configFile) : '';

preg_match('/用戶端 ID:\s*([^\r\n]+)/u', $configText, $clientIdMatch);
preg_match('/用戶端密碼:\s*([^\r\n]+)/u', $configText, $clientSecretMatch);

$defaultRedirectUri = 'http://localhost/Cybersecurity-Defense-Training-System/api/oauth/google_callback.php';
